...
## Revisit (DO NOT AUTOMATICALLY ADDRESS ANYTHING IN THIS SECTION)
...
## Design & Style (DO NOT AUTOMATICALLY ADDRESS ANYTHING IN THIS SECTION)
...
- [x] Use the table web app diagram for illustrating how to reserve space in the header/data table
      view example.
- [x] Client side rendering seems simpler when we're not worried about the details. We're not
...
- [x] Simplicity (Done 2026-07-19: all three woven into §4.2's Simplicity subsection: an
  ...
  - [x] How CSR forces you to choose between using fallback values OR not revealing any content
...
- [x] Discussion of the nextjs page cache or html cached output and how that relates to the server
      content that is maximized. (Checked 2026-07-19: already covered by §4.4 "The cached page:
      maximized server content becomes a cacheable artifact".)
...
- [x] The same three reads, three shapes - diagram needs to be better. It should incorporate an
      understanding of how suspense can parallelize server reads and how the server reads are not
      longer than the client ones.
- [x] More content on forms and actions - maybe on its own page. Zero in on the new Form component
      from NextJS, the useFormStatus, the useActionStatus, and how action prop on Form works with
      server actions. (Done 2026-07-19: new §5.11 "Forms and actions, end to end" right after the
