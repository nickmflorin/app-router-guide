...
- **STYLES ARE SCSS (2026-07-19, per Nick): the source of truth is `src/styles/style.scss` +
  `src/styles/partials/_*.scss` (split by design-system section). `public/assets/style.css` is
  GENERATED by `npm run css` (scripts/build_css.mjs) and is gitignored: NEVER edit it directly.
  `npm run dev` and `npm run build` both compile css first. build_css.mjs uses dart-sass (the
  `sass` devDependency) when installed; in environments without it (the sandbox registry blocks
  the package) it falls back to deterministic @use-inlining, which handles the plain-CSS subset
  and FAILS LOUDLY if a partial uses SCSS-only syntax. The @dev-only markers live at the top and
  bottom of _draft-tools.scss and must survive compilation (they do: /* */ comments are kept).
  `npm run css:watch` = live recompile during styling work (needs sass installed).**
