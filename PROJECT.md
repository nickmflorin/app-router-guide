...
- Static assets live in `public/assets/` (nav.js, logomark.svg) → copied into
  build/output/app-router-guide_html/assets at build.
- **STYLES ARE SCSS (2026-07-19, per Nick): the source of truth is `src/styles/style.scss` +
  `src/styles/partials/_*.scss` (split by design-system section). As of 2026-07-22 the SCSS is
  imported directly from `src/layouts/GuidePage.astro`, so VITE (dart-sass, the `sass`
  devDependency) compiles it: live HMR in `npm run dev`, emitted into `_astro/*.css` by `astro
  build`. There is NO separate CSS build step — `build_css.mjs`, `npm run css`, `css:watch`, and
  the generated `public/assets/style.css` are all gone. Editing any partial hot-reloads in the
  browser with no server restart. The dev-only draft layer (`_draft-tools.scss`) is NOT `@use`d by
  `style.scss`; GuidePage imports it separately under `import.meta.env.DEV`, so the production build
  dead-code-eliminates it (this replaces the old @dev-only marker-strip on style.css — nav.js still
  uses the markers). Vite needs `sass` installed; in a sandbox without it, `astro build` fails
  loudly rather than silently shipping unstyled output.**
...
- **DIAGRAM COLORS ARE TOKENS (2026-07-21, per Nick).** SVG diagrams no longer use inline
  ...
  twins keep their own `classDef fill:#..` hex (Mermaid can't read CSS vars) — leave them.
  New diagrams: use fill-dg-*/stroke-dg-* classes, not hex; add a token if a color is new.
...
- 2026-07-18: **Draft layer is DEV-ONLY (per Nick).** The built build/output/app-router-guide_html/ is FINAL, never a
  ...
  ledger auto-sync) behind a localhost check, AND the module + draft-tools CSS are kept out of
  the BUILT assets by two mechanisms: postbuild_relativize.py removes everything between
  /* @dev-only:start */ ... /* @dev-only:end */ markers in nav.js (keep the markers intact when
  editing that file; the build FAILS if they go missing), and _draft-tools.scss is imported only
  under import.meta.env.DEV in GuidePage.astro so `astro build` dead-code-eliminates it from
  _astro/*.css. postbuild also deletes page-notes.json from the output (also gitignored there).
  The working ledger stays at
...
- **No fragment roots in example snippets (per Nick, 2026-07-20):** layout/page examples return a
  real container (usually a flex-col div; frames use max-h-full + overflow-hidden with children
  implementing their own scroll), never a bare <>...</> at the root.
