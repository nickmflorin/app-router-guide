...
`npm run build` chains: `astro build` (compiles the SCSS into `_astro/`),
`scripts/postbuild_relativize.py` (relativizes asset URLs so `file://` works, deletes the notes
ledger from the output, and strips the dev-only block from `nav.js`; it fails the build if the strip
markers are missing), prettier over the output, and `scripts/build_search_index.py` (writes
`search-index.js` into both the build and `public/assets/` so dev-server search works).
